...
/**
 * MongoProtoUser is a base class that gives you a "User" that has a first name,
 * last name, email, etc.
 */
trait MongoProtoUser[T <: MongoProtoUser[T]] extends MongoRecord[T] with UserIdAsString with MongoId[T]{
  self: T =>
...
trait MongoMetaMegaProtoUser[ModelType <: MongoMegaProtoUser[ModelType]] extends MongoMetaRecord[ModelType] with MongoId[ModelType] with GenProtoUser  {
  self: ModelType =>
...
/**
 * MongoProtoUser is bare bones.  MetaProtoUser contains a bunch
